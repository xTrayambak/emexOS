...
static void run_cmd(void)
{
    ...
    if (!strcmp(argv[0], "exec")) {
        if (!argv[1]) { buf_puts("exec: path required\n", CI_ERR); return; }
    ...
    if (bl + cl + 5 > BUFFER) { buf_puts(argv[0], CI_ERR); buf_puts(": name too long\n", CI_ERR); return; }
    memcpy(path, BIN_PATH, bl);
    memcpy(path + bl, argv[0], cl);
    memcpy(path + bl + cl, ".elf", 5);

    if (!file_exists(path)) { buf_puts(argv[0], CI_ERR); buf_puts(": command not found\n", CI_ERR); return; }

    char *const envp[] = { (char *)0 };
    pid_t pid = fork();
    if (pid == 0) { execve(path, argv, envp); _exit(127); }
    else if (pid > 0) { waitpid(pid, NULL, 0); redraw(); }
    else { buf_puts("fork failed\n", CI_ERR); }
}
...
int main(void)
{
    ...
    for (int i = 0; i < TBUF_ROWS; i++) { t_text[i][0] = '\0'; memset(t_clr[i], 0, TBUF_COLS); }
    ...
    for (;;)
    {
        ...
        if (kc == '\n' || kc == '\r') {
            ...
            if (ilen > 0) { ilen--; ibuf[ilen] = '\0'; buf_putc('\b', CI_FG); redraw(); }
        } else if (c >= 0x20 && c <= 0x7E) {
            if (ilen < BUFFER - 1) { ibuf[ilen++] = c; ibuf[ilen] = '\0'; buf_putc(c, CI_FG); redraw(); }
        }
    }
}