...
d_boot_screen: { // Initializing Boot screen
  theme_init();
  setcontext(
      THEME_BOOTUP); // gets loaded over sbootup_theme until, sbootup == FLU
  ...
  // emexOS start
  // Ensure that Limine base revision is supported and that we have a
  // framebuffer
...
#if ENABLE_ULIME
  u64 phys_ulime =
      map_region_alloc(hhdm_request.response, ULIME_START, ULIME_META_SIZE);
#if ENABLE_GLIME
...
kernel_slot_ss: { dualslotvalidating(); }
